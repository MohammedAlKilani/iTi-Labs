...
import { User, UserDaoInterface, UserSignUp, UserUpdate } from "../interfaces/user.interface";
...
export default class UserDao implements UserDaoInterface{
    ...
    async updateUser(id: Schema.Types.ObjectId, user: UserUpdate): Promise<User | null> {
        if(user.password){
