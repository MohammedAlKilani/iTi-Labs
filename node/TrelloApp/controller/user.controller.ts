...
import { UserSignUp ,UserRes, UserLogin, User, UserUpdate } from "../interfaces/user.interface";
...
export const signUp =tryCatchErr<UserSignUp,ResInterface<UserRes>>(async (req,res)=>{
const user = req.body
...
export const login = tryCatchErr<UserLogin,ResInterface<UserRes>>(async (req,res)=>{

   const userFind = await userDao.findUserByEmail(req.body.email)
   
   if(!userFind)return res.status(404).json({message:`Not found user with email ${req.body.email}`})
   if(userFind){
      if(userFind?.softDelete) return res.json({message:`user is soft Delete`})
      if(!userFind?.isVerified) return res.json({message:`email ${userFind?.email} not Verified`})
   const compared =  await bcrypt.compare(req.body.password,userFind.password)
   if(compared){
...
export const Verifi = tryCatchErr<never,ResInterface<UserRes>,{token:string}>(async (req,res)=>{
 ...
 const data =userVerifie as UserRes
   data.password = undefined
 const tokenData = jwt.sign(data.toJSON(), process.env.SECRET_KEY!) as unknown as UserRes
...
export const logout = tryCatchErr<never,ResInterface<never>>(async (req,res)=>{
   const token =  req.cookies?.token
   if(token)return res.status(403).json({message:"you are logOut"})
